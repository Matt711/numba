...
def hllines_llvm_ir(code, style):
    try:
        from pygments import highlight
        from pygments.formatters import HtmlFormatter
...
def htlines_llvm_ir(code, style):
    try:
        from pygments import highlight
        from pygments.formatters import TerminalFormatter
        from pygments.lexers import LlvmLexer
...
def get_html_template_llvm():
    ...
    <body>
        {% for func_key in func_data.keys() %}
            ...
            <table class="annotation_table tex2jax_ignore">
                {%- for num, line, hl, hc in func_data[func_key]['pygments_lines'] -%}
                    {%- if func_data[func_key]['llvm_ir_lines'][num] %}
                        <tr><td style="text-align:left;" class="{{func_data[func_key]['python_tags'][num]}}">
                            <details>
                                <summary>
                                    <code>
                                ...
                                <table class="annotation_table">
                                    <tbody>
                                        {%- for ir_num, ir_lines, ir_hl, ir_hc in func_data[func_key]['pygments_llvm_ir_lines_'+num|string] %}
                                            <tr class="ir_code">
                                                <td style="text-align: left;">
                                                    <code>
                                                        {{'&nbsp;'*func_data[func_key]['llvm_ir_indent'][num][ir_num-1]}}{{ir_hl}}
                                                    </code>
                                                </td>
                                            </tr>
                                        {%- endfor -%}
                    ...
                    {%- if func_data[func_key]['llvm_ir_lines'][num] %}
                        <tr><td style="text-align:left;" class="{{func_data[func_key]['python_tags'][num]}}">
                            <details>
                                ...
                                <table class="annotation_table">
                                    <tbody>
                                        {%- for ir_line in func_data[func_key]['llvm_ir_lines'][num] %}
                                            <tr class="ir_code">
                                                <td style="text-align: left;">
                                                    <code>
                                                        {{'&nbsp;'*func_data[func_key]['llvm_ir_indent'][num][loop.index0]}}{{ir_line}}
                                                    </code>
                                                </td>
                                            </tr>
                                        {%- endfor -%}
...
def reform_llvm_ir_code(annotation, i):
    """
    Extract the code from the Numba annotation datastructure. 
    ...
    annotation is list of single lines, with indentation removed.
    """
    ident_dict = annotation['llvm_ir_indent']
    s= ''
    for n,l in annotation[f'llvm_ir_lines_{i}']:
        try:
            s = s+' '*ident_dict[i][n-1]+l+'\n'
        except TypeError:
...
def seperate_llvm_ir_lines(annotation):
    ir = list(annotation.items())[0][1]['llvm_ir_lines']
    lines = [l for l in ir.keys() if ir[l] != []]
    for i in lines:
        list(annotation.items())[0][1][f'llvm_ir_lines_{i}'] = [(j+1, l[0]) if j!= 0 else (j+1, l) for j, l in enumerate(ir[i])]
...
class AnnotateLLVM:
    def __init__(self, function, signature=None, **kwargs):
        ...
        self.python_lines = kwargs.get('python_lines', None)
        self.python_indent = kwargs.get('python_indent', None)
        self.llvm_lines = kwargs.get('llvm_ir_lines', None)
        self.llvm_indent = kwargs.get('llvm_ir_indent', None)
        ...
        if self.python_indent:
            list(ann.items())[0][1]['python_indent'] = self.python_indent
        if self.llvm_lines:
            list(ann.items())[0][1]['llvm_ir_lines'] = self.llvm_lines
        if self.llvm_indent:
            list(ann.items())[0][1]['llvm_ir_indent'] = self.llvm_indent
        ...
        import math
        list(ann.items())[0][1]['N'] = f"%0{int(math.log10(len(list(ann.items())[0][1]['python_lines']))+1)}d"
        seperate_llvm_ir_lines(ann)
        ir = list(ann.items())[0][1]['llvm_ir_lines']
        lines = [l for l in ir.keys() if ir[l] != []]
        for i in lines:
            for k,v in ann.items():
                res = hllines_llvm_ir(reform_llvm_ir_code(v, i), style)
                rest = htlines_llvm_ir(reform_llvm_ir_code(v, i), style)
                v[f'pygments_llvm_ir_lines_{i}'] = [(a,b,c,d) for (a,b),c,d in zip(v[f'llvm_ir_lines_{i}'], res, rest)] # Batch into pygments_ir_lines_1, 2, ...

    def _repr_html_(self):
        return get_html_template_llvm().render(func_data=self.ann)
...
class AnnotatePTX:
    def __init__(self, function, signature=None, **kwargs):
        ...
        for k,v in ann.items():
            v['ptx_lines'] = [(a,b) for (a,b) in v['llvm_ir_lines'].items()]
...
class AnnotatePTX2:
    ...
    def _repr_html_(self):
        return get_html_template_ptx2().render(func_data=self.ann)